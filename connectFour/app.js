document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkBoard() {
        ...
        //now take values in each winningArrays and plug them into the squares
        for (let y = 0; y < winningArrays.length; y++) {
            const square1 = squares[winningArrays[y][0]]
            const square2 = squares[winningArrays[y][1]]
            const square3 = squares[winningArrays[y][2]]
            const square4 = squares[winningArrays[y][3]]
            //now check to see if those arrays all have the class of player-one
            if (square1.classList.contains('player-one') &&
                square2.classList.contains('player-one') &&
                square3.classList.contains('player-one') &&
                square4.classList.contains('player-one')) {
                //player one passed as winner
                result.innerHTML = 'Player One Wins'
            }
            // now check to see if those arrays all have the class of player-two
            else if (square1.classList.contains('player-two') &&
                square2.classList.contains('player-two') &&
                square3.classList.contains('player-two') &&
                square4.classList.contains('player-two')) {
                //player two passed as winner
